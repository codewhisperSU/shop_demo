...
describe('Test product controller', () => {
    ...
    it('Get error when unit price missing! ', async () =>{
        ...
        (productService.createProduct as jest.MockedFunction<any>).mockResolvedValueOnce();
        (productService.getListOfProduct as jest.MockedFunction<any>).mockResolvedValueOnce([{ name: "Test product", unit_price: 120 }]);
         ...
         const customerRequest = {
          ...
          await customerController.addProduct(customerRequest);
            ...
            expect((ex as {message:string}).message).toBe("Name or unit price missing!");
          }  
    });
